...
export function fillBlanks(text: string[], values: string[]) {
  let completed = "";
  for (let i = 0; i < text.length; i++) {
    if (i < values.length) {
      completed += text[i] + values[i];
    } else {
      completed += text[i];
    }
  }
...
// deprecated
// function fetchDataFromURL(link: string) {
//   const xhttp = new XMLHttpRequest();
//   const data: Array<intf.MadLib> = [];
...
// This is the fetchData function that's actually used
function fetchDataLocal() {
  const myObj = data as intf.MadLibJSON;
  const keys = Object.keys(myObj);
  const randIndex = Math.floor(Math.random() * keys.length);
  const choice = myObj[keys[randIndex]];

  const returnObj: intf.MadLib = {
    blanks: choice.blanks,
    image: choice.image,
    text: choice.text,
    title: keys[randIndex],
  };
  return returnObj;
}

// deprecated
// function fetchDataHardCoded() {
//   const myObj = {
...
//   return myObj as intf.MadLibJSON;
// }

// fetchData("http://madlibz.herokuapp.com/api/random?minlength=5&maxlength=25");
